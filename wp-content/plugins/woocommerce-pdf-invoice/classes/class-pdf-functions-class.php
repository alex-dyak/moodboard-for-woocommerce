        ...
        class WC_pdf_functions {
            ...
            public function __construct() {
            	...
            	if ( extension_loaded('iconv') ) {
		            ...
		            // custom filter
		            add_action( 'admin_notices', array($this, 'invoices_download_filter') );
	        ...
	        /**
	         * Add monthly filters for invoices
	         */
	        function invoices_download_filter() {
		        global $wpdb, $wp_locale, $current_screen;
		        if($current_screen->post_type != 'shop_order'){
			        return;
		        }
		        $months = $wpdb->get_results("SELECT DISTINCT YEAR( meta_value ) AS year, MONTH( meta_value ) AS month FROM $wpdb->postmeta
		        ...
		        <div id="inv_filters">
			        <form method="POST">
				        <select name="yearmonth" id="inv_yearmonth_filter">
					        <?php
				        ...
				        <input type="submit" id="inv_filter_submit" class="button"  value="<?php _e('Print Invoices for this month', 'woocommerce-pdf-invoice'); ?>" />

			        </form>
		        </div>

		        <?php
		        add_action( 'admin_footer', array( $this, 'inv_admin_footer' ) );
	        }

	        public function inv_admin_footer() {
		        ?>
		        <script type="text/javascript">
			        jQuery(document).ready(function($){
				        if($('#inv_filters').length && $('ul.subsubsub').length){
					        $('#inv_filters').insertBefore('ul.subsubsub');
				        }
			        });
		        </script>
	        <?php
			 ...
			 function admin_pdf_url_check() {
				 ...
				 if ( is_admin() && isset( $_POST['yearmonth'] ) && ! empty( $_POST['yearmonth'] ) ) {
					 echo WC_send_pdf::get_woocommerce_monthly_invoices( $_POST['yearmonth'] );
				 }

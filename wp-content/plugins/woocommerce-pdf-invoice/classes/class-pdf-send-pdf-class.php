<?php
        ...
        class WC_send_pdf {
	        ...
	        Public Static function get_woocommerce_monthly_invoices( $yearmonth = null) {
		        if ( empty( $yearmonth ) ) {
			        return;
		        }
		        global $woocommerce;
		        ...
		        $monthly_invoices = $wpdb->get_results("SELECT post_id FROM $wpdb->postmeta
														WHERE meta_key = '_invoice_date'
														AND extract(YEAR_MONTH FROM meta_value) = $yearmonth
														ORDER BY meta_value DESC;"
		        );
		        if(empty($monthly_invoices)){
			        return;
		        }
...
//				spl_autoload_register('DOMPDF_autoload');
		        ...
		        if ( !class_exists('PDFMerger') )
			        include( WP_PLUGIN_DIR
			                 . '/woocommerce-pdf-invoice/lib/PDFMerger/PDFMerger.php' );
		        ...
		        foreach($monthly_invoices as $invoice){
			        ...
			        $filename	= 'tmp_invoice_' . $invoice->post_id;

			        $messagetext  = '';
			        $messagetext .= $pdf->get_woocommerce_invoice_content( $invoice->post_id );
			        ...
			        $pdf_m->addPDF( $termsattachments, 'all' );
		        }

		        $filename	= __('invoices') . '-' . $yearmonth . '.pdf';

		        $pdf_m->merge( 'download', $filename );
		        exit;
	        }
	        ...
	        function get_woocommerce_invoice_content( $order_id ) {
		        ...
		        $headers =  '<table class="shop_table orderdetails" width="100%">' .
		                    '<thead>' .
		                    '<tr><th colspan="4" align="left"><h2>' . esc_html__('Order Details', PDFLANGUAGE) . '</h2></th></tr>' .
		                    '<tr>' .
		                    '<th width="5%" valign="top" align="left">'  . __( 'Qty', PDFLANGUAGE )        . '</th>' .
		                    '<th width="75%" valign="top" align="left">'  . __( 'Product', PDFLANGUAGE )    . '</th>' .
		                    '<th width="10%" valign="top" align="left">' . __( 'Price (' . $order->get_order_currency() . ')', PDFLANGUAGE )  . '</th>' .
		                    '<th width="10%" valign="top" align="left">' . __( 'Total (' . $order->get_order_currency() . ')', PDFLANGUAGE )  . '</th>' .
		                    '</tr>' .
		                    '</thead>' .
		                    '</table>';

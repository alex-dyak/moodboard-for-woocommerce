...
*/
define( 'WOO_MULTI_CURRENCY_VERSION', '1.5.2' );
...
class Woo_Multi_Currency {
	 ...
	 * @return mixed
	 */
	public function woocommerce_product_get_sale_price( $price, $obj ) {
		return wmc_get_price( $price );
	}
	 ...
	 */
	public function wmc_get_currency_symbol( $currency_symbol ) {
	 ...
	 */
	public function wmc_get_price_format( $format ) {
		if ( is_order_received_page() ) {
			global $wp;
			$order_id    = $wp->query_vars['order-received'];
			$order       = wc_get_order( $order_id );
			$currency    = $order->get_currency();
			$current_pos = $this->selected_currencies[ $currency ]['pos'];
		} elseif ( array_key_exists( $this->current_currency, $this->selected_currencies ) ) {

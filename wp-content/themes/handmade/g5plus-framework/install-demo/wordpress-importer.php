...
if (!defined('G5PLUS_THEME_MOD_NAME')) {
	define('G5PLUS_THEME_MOD_NAME','handmade');
}
...
define( 'CHANGE_DATA_FILE', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . $demo_site_install . DIRECTORY_SEPARATOR . 'change-data.json' );

define( 'LOG_IMPORT_FOLDER',get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log');
define( 'LOG_PROCESS_POST', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' .DIRECTORY_SEPARATOR . 'log_process_post.log' );
define( 'MENU_MAPPING', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'menus.log' );
define( 'MENU_ITEM_ORPHANS', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'menu_item_orphans.log' );
define( 'PROCESS_TERM', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'process_term.log' );
define( 'PROCESS_POSTS', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'process_posts.log' );
define( 'MENU_MISSING', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'menu_missing.log' );
define( 'URL_REMAP', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'url_remap.log' );
define( 'POST_ORPHANS', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'post_orphans.log' );
define( 'FEATURE_IMAGES', get_template_directory() . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'data-demo' . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'feature_images.log' );
...
require THEME_DIR . 'g5plus-framework/install-demo/parsers.php';
...
if ( class_exists( 'WP_Importer' ) ) {
	class G5_Import extends WP_Importer {
		...
		function import( $file ) {
			...
			if (file_exists(MENU_MAPPING)) {@$this->processed_menu_items = json_decode( $wp_filesystem->get_contents( MENU_MAPPING ), true );}
			if (file_exists(PROCESS_TERM)) {@$this->processed_terms = json_decode( $wp_filesystem->get_contents( PROCESS_TERM ), true );}
			if (file_exists(PROCESS_POSTS)) {@$this->processed_posts = json_decode( $wp_filesystem->get_contents( PROCESS_POSTS ), true );}
			if (file_exists(MENU_ITEM_ORPHANS)) {@$this->menu_item_orphans = json_decode( $wp_filesystem->get_contents( MENU_ITEM_ORPHANS ), true );}
			if (file_exists(MENU_MISSING)) {@$this->missing_menu_items = json_decode( $wp_filesystem->get_contents( MENU_MISSING ), true );}
			if (file_exists(URL_REMAP)) {@$this->url_remap = json_decode( $wp_filesystem->get_contents( URL_REMAP ), true );}
			if (file_exists(POST_ORPHANS)) {@$this->post_orphans = json_decode( $wp_filesystem->get_contents( POST_ORPHANS ), true );}
			if (file_exists(FEATURE_IMAGES)) {@$this->featured_images = json_decode( $wp_filesystem->get_contents( FEATURE_IMAGES ), true );}
			...
			if ($process_post_return !== true) {
				...
				if ( $this->processed_menu_items ) {
					$wp_filesystem->put_contents( MENU_MAPPING, json_encode( $this->processed_menu_items ), FS_CHMOD_FILE );
				}
				if ( $this->processed_terms ) {
					$wp_filesystem->put_contents( PROCESS_TERM, json_encode( $this->processed_terms ), FS_CHMOD_FILE );
				}
				if ( $this->processed_posts ) {
					$wp_filesystem->put_contents( PROCESS_POSTS, json_encode( $this->processed_posts ), FS_CHMOD_FILE );
				}
				if ( $this->menu_item_orphans ) {
					$wp_filesystem->put_contents( MENU_ITEM_ORPHANS, json_encode( $this->menu_item_orphans ), FS_CHMOD_FILE );
				}
				if ( $this->missing_menu_items ) {
					$wp_filesystem->put_contents( MENU_MISSING, json_encode( $this->missing_menu_items ), FS_CHMOD_FILE );
				}
				if ( $this->url_remap ) {
					$wp_filesystem->put_contents( URL_REMAP, json_encode( $this->url_remap ), FS_CHMOD_FILE );
				}
				if ( $this->post_orphans ) {
					$wp_filesystem->put_contents( POST_ORPHANS, json_encode( $this->post_orphans ), FS_CHMOD_FILE );
				}
				if ( $this->featured_images ) {
					$wp_filesystem->put_contents( FEATURE_IMAGES, json_encode( $this->featured_images ), FS_CHMOD_FILE );
				}
		...
		function process_posts() {
			...
			if (file_exists(LOG_PROCESS_POST)) {
				$start_post = $wp_filesystem->get_contents(LOG_PROCESS_POST);
			}
			...
			foreach ( $this->posts as $current_post => $post ) {
				...
				if ($current_post >= $end_post) {
					$wp_filesystem->put_contents(LOG_PROCESS_POST, $end_post, FS_CHMOD_FILE);
					return $process_post_return;
				}
